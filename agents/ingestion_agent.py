...
Loads and validates the uploaded proteomics file (canonical 2-sheet template:
Sample ID/Group metadata + Protein/Accession/Gene/sample intensity columns)
and surfaces dataset metadata in BiomarkerState for downstream analysis.
...
class IngestionAgent(BaseAgent):
    ...
    The supported workbook format is:
      ...
      • Sheet 2 (proteins)  : Protein Name | Accession Number | Gene Name | <samples>
    The agent extracts a sample→group map from Sheet 1 and propagates it to
    state so BiomarkerAgent can run the dual-engine (Python + R/limma)
    ...
    def run(self, state: BiomarkerState) -> BiomarkerState:
        ...
        # ── omic_type routing ─────────────────────────────────────────────────
        # The only registered omic skill is "proteomics" — the pooled-vs-Welch
        # decision is now made inside ProteomicsAnalysisSkill via the
        # is_pooled_design flag. Honour any explicit hint (only "proteomics"
        # is valid today), otherwise default.
        hinted = result.get("omic_type")
        state["omic_type"] = "proteomics" if hinted in (None, "proteomics_pooled",
